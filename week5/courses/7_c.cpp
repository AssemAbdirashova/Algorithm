...
void rec(long long int i, vector<long long int>& v, vector<long long int>& ans, long long int target, vector<long long int> & result){
    if(target == 1){
...
int main(){
    ...
    long long int k = sqrt(maxx);
    v = fib(k);
    //cout << v[0] << v[1] << v[2] << v[3];
    for(long long int i = 0; i < n; i++){
        vector<long long int> ans;
        vector<long long int> result;
        rec(0, v, ans, candidates[i], result);
        if(!result.empty()){
